...
mqFile <- "../../extdata/specht2019v1/ev_updated.txt"
...
batch <- read.csv("../../extdata/specht2019v1/batch_fp60-97.csv", row.names = 1)
annot <- read.csv("../../extdata/specht2019v1/annotation_fp60-97.csv", 
                       row.names = 1, check.names = FALSE)
sampleMap(scp)[, c("LcBatch", "SortBatch", "DigestBatch")] <- 
  batch[as.character(sampleMap(scp)$assay), ]
sampleMap(scp)$SampleType <- annot[sampleMap(scp)$colname, exp]


####---- Filter features ----####

## TODO make issue on github?
if(F){
  ## data.frame format
df <- df[df$Reverse != "+", ]
df <- df[df$Potential.contaminant != "+", ]
df <- df[!grepl("^REV", df$Leading.razor.protein), ]
df <- df[df$PIF > 0.8 & !is.na(df$PIF), ]
## Features format
fts <- filterFeatures(fts, ~ Reverse != "+")
fts <- filterFeatures(fts, ~ Potential.contaminant != "+")
## Problem 1: how to do if need to "grep"
## Problem 2: PIF contains NA, method throws "Error: logical subscript contains NAs"
## Idea
## filt <- VariableFilter(field = "Leading.razor.protein",
##                        fun = funtion(x) !grepl("^REV", x))
## fts <- filterFeatures(fts, filter = filt)
}
...
for (exp in names(scp)) {
  scp <- aggregateFeaturesSCE(scp, 
                              i = exp, 
                              fcol = "Sequence", 
                              name = paste(exp, "_peptides"), 
                              fun = colMeans)
  print.progress(which(names(scp) == exp), length(scp))
}
